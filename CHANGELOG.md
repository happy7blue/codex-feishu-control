...
- 添加 MIT License，明确公开仓库的使用、修改和分发授权。
- 重写 README，说明飞书通知 Hook / cron 部署后本机运行，不依赖大模型；只有通过守护进程创建新 Codex 任务时才会调用 `codex exec`。
- 补充敏感信息规则，并将 `backups/`、`*.env`、`*.bak` 加入忽略范围，避免本地备份和凭据误提交。
